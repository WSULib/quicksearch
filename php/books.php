...
if ($data_type == "xml2json"){
	...
	if (!$xml){ // if finding the IIIRecord borks, then fall back to a simpler search and ignore the LC call number and any stackview/view on shelf
	...
	foreach ($books['bibNum'] as $key => $value) {
		...
		if (substr($value, 0, 1) === "e") {
			...
			$title = trim(str_replace("&nbsp;", '', $title->plaintext));
			$lc = null;
			$status = null;
			$location = null;
			$showStackView = "no";
		...
		$books['title'][$key] = $title;
		$books['location'][$key] = $location;
		$books['lc'][$key] = $lc;
...
// OLD  -- HAS Useful parsing for z3950 and XMLserver
