<?php $wsuroot = realpath($_SERVER["DOCUMENT_ROOT"]); $headerType = "plain"; $pageTitle=""; $pageBreadCrumbsGoByURL = "resources/quicksearch/api/QueryParser_old_jan4.php"; $pageSidebarSectionsGoByURL = "resources/quicksearch/api/QueryParser_old_jan4.php"; require_once("$wsuroot/header.php") ?><?php require_once("$wsuroot/page-begin.php"); ?><?php
...
// ini_set('display_errors', 1);
			...
			if (false && !$api['Title'][0]) {
				//echo 'beyondcollecitoin';
			...
			if (isset($api['Title'][0])) {
				...
				$this->bestResult['title'] = $this->replaceWithBold($this->queryKeywords, $api['Title'][0]);
	...
	public function getKeywords($query) {
		$arr = array_unique( explode(" ", preg_replace("/[^A-Za-z0-9 ]/", "", $this->queryNormalizedWhitespace) ) );// we only want to replace each term once
		// remove emptys
...
class IdentifierParser {
	...
	public function __construct($query) {
		...
		if (preg_match("/^([0-9]{7}|ocm[0-9]{8}|om[0-9]{8}|ocn[0-9]{9}|on[0-9]{10,})$/i", $query, $match)) {
			// OCLC
		...
		if (preg_match("/^(10[.][0-9]{4,}(?:[.][0-9]+)*\/(?:(?![%\"#? ])\\S)+)$/i", $query, $match)) {
			...
			if ($api['Title'][0]) {
				$results[] = array(
					...
					'url' => $api['link'],
					'image' => $this->validImage( $api['thumbnail_m'][0], 'http://www.syndetics.com/index.aspx?issn=' .$api['EISSN'][0]. '/sc.gif&client=waynestmis&freeimage=true' ),
					'data' => array(
						'Authors' => implode(', ', $api['Author']),
						'Date' => date("n/j/Y", strtotime($api['PublicationDate'][0])),
						'Publication' => $api['PublicationTitle'][0],
